...
for i in range(T):  # 테스트 케이스 개수만큼 반복한다
    ...
    for j in range(2, len(n_list) - 1):  # 왼쪽, 오른쪽 2칸은 모두 0층이므로 범위를 이렇게 결정
        ...
        if n_list[j - 2] < n_list[j] and n_list[j - 1] < n_list[j] and n_list[j] > n_list[j + 1] and n_list[j] > n_list[j + 2]:  # 앞 1, 2칸 뒤 1,2칸보다 가운데 층이 크다면 조망권이 확보된 것이므로
            ...
            for bubble in range(len(neighborhood_list)):  # 앞 뒤 1,2칸 들을 버블정렬을 통해 오름차순 시킨다
                for sort in range(bubble+1, len(neighborhood_list)):
                    if neighborhood_list[bubble] > neighborhood_list[sort]:
                        neighborhood_list[bubble], neighborhood_list[sort] = neighborhood_list[sort], neighborhood_list[bubble]
