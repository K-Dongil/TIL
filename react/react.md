...
- Hook API
  ...
  - useMemo & useCallback
    ...
    - useMemo : 계산량이 많은 **함수의 반환값**을 재활용하는 용도로 사용
      - props 변경이 안 된 Component는 재렌더링 하지 않도록 해준다
...
- App 컴포넌트의 렌더링 결과물에 영향을 주는 데이터를 갖고 있는 객체
  - 컴포넌트 안에서 관리된다.
  - 사용하는 쪽과 구현하는 쪽을 철저하게 분리
...
- 페이지 이동하기 위해 쓰는 태그
  ...
  - 브라우저의 주소만 바꿀 뿐, 페이지를 새로 불러오지는 않는다.(새고고침X)
  - a태그는 브라우저의 주소를 이동하며 페이지 자체를 새로고침한다.
...
1. reducer 함수생성
   ...
   - 항상 state(수정 or 기본 state)를 return
     - return으로 반환하는 값이 redux에 저장되는 값이다.
